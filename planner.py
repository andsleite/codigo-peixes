...
class Planner():
  ...
  # Função -> Thread da notificação
  def escutarNotificacoes(self, listaEvento):

    def fn():
      while True:
        time.sleep(1)
        self.enviarNotificacao(listaEvento)

    thread = Thread(target=fn, name='EscutarPorNotificaoes')
    thread.start()

  # Função -> Calcula tempo da notificação
  def enviarNotificacao(self, listaEvento):
    for evento in listaEvento:
      atual = dt.now()
      diferenca = evento.getData() - atual
      total = diferenca.days
      total_segundos = diferenca.seconds
      _1_minuto = 60
      _5_minutos = _1_minuto * 5
      _20_minutos = _1_minuto * 20
      _30_minutos = _1_minuto * 30
      _1_hora = _30_minutos * 2
      _1_dia = _1_hora * 24

      if total_segundos <= _20_minutos:
        evento.alertarNotificacao()
